...
## demographics
```{r}
duplicate_ids = read.csv("../data/demographics/MURI_Master_Participants - Overview.csv", stringsAsFactors = FALSE) %>%
  filter(duplicate > 0) %>%
...
completion_data = read.csv("../data/demographics/cleaned_completion_data.csv") %>%
  spread(key, value)

demo_all = read.csv("../data/demographics/master_demo.csv") %>%
...
## task
```{r}
task = read.csv("../data/task/cuereact_all062920.csv", stringsAsFactors = FALSE) %>%
